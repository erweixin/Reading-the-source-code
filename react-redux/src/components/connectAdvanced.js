  ...
  */
  ...
  return function wrapWithConnect(WrappedComponent) {
    ...
    function makeDerivedPropsSelector() {
      ...
      let lastStore
      let sourceSelector
      // 根据state、props、store、selectorFactoryOptions获取需要传给子组件的props。
      return function selectDerivedProps(state, props, store) {
        ...
        if (store !== lastStore) {
          lastStore = store
          // selectorFactory返回一个以state、props为参数的函数，该函数返回值为需要传递给子组件的props(包含connect api中mapStateToProps和mapDispatchToProps中生成的props).
